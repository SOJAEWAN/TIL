...
# 문제에서는 학생들과 점수는 달라질 수 있지
# 만, 항상 위와 같은 형태로 올바르게 해석될 수 있는 
...
# result = []
# def func(solution, sybj, cut):
       
#         for key, value in solution.items():
#                 if value.get(sybj) >= cut:
#                         result.append(key)
#                 return sorted(result)        
                
# func(dict_repr, 'math', 70)
# print(result)           
# new = []         


def func(solution, key, cut):
    result = []
    for key1, val in dict_repr.items():
        if val[key] > cut:
            result.append(key1)
    return sorted(result)    

print(func(dict_repr, 'Math', 70))



def func(solution, key, cut):   
    return sorted([key1 for key1, val in solution.items() if val[key] > cut])               
print(func(dict_repr, 'Math', 70))    
